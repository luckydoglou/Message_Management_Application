package ser321.assign3.xhuan127.client;

import ser321.assign3.xhuan127.server.*;
...
public class Client extends MessageGui
    ...
    public Client(String userId, String hostId, String regPort) {
        ...
        try{
            // find the server from RMI registry
            server = (Server_MessageLibraryInterface) Naming.lookup(
                "rmi://"+hostId+":"+regPort+"/Server");
            System.out.println("Client: " + userId + " obtained remote object reference to" +  "the Server at:\n"+ "rmi://" + hostId + ":" + regPort + "/Server");
        } catch(Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            msgHeaders = server.getMessageFromHeaders(userId);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public void valueChanged(ListSelectionEvent e) {
        ...
        if (!e.getValueIsAdjusting()){
            ...
            if (messageListJL.getSelectedIndex() > -1) {
                ...
                DefaultListModel<String> dlm = (DefaultListModel<String>)messageListJL.getModel();
                try {
                    Message msg = server.getMessage(dlm.getElementAt(messageListJL.getSelectedIndex()), userId);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getActionCommand().equals("Exit")) {
            ...
            try {
                if (server.deleteMessage(msgHeaders[selected], userId)) {
            ...
            try {
                Message msg = server.getMessage(dlm.getElementAt(messageListJL.getSelectedIndex()), userId);
                server.deleteMessage(msgHeaders[selected], userId);
                ...
                boolean isSent = server.sendClearText(msg, msg.getMessageTo());
                if (isSent) {
            ...
            try {
                msgHeaders = server.getMail(userId);
